...
def csvs(request):
    ...
    for x in fsa[1:]:
        try:
            robores = requests.get(x+'/robots.txt')
        except Exception as e:
            print(f"NOT OK: {str(e)}")
            robot = f"NOT OK: {str(e)}"
            roboeasy.append(robot)
        else:
            if robores.status_code == 200:
                print("OK")
                robot = "OK"
                roboeasy.append(robot)
            else:
                print(f"NOT OK: HTTP response code {robores.status_code}")
                robot = f"NOT OK: HTTP response code {robores.status_code}"
                roboeasy.append(robot)
                #print(x)
    print(roboeasy)
